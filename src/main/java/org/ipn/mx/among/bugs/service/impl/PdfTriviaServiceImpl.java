...
public class PdfTriviaServiceImpl implements PdfTriviaService {
	...
	public byte[] generatePlayerStatsReport(Long playerId) {
		try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			if (attempts.isEmpty()) {
				final String[] args = {player.getUsername()};
				final String message = messageSource.getMessage("stats.player.not.found", args, locale);
			...
			// Título del reporte
			Paragraph mainTitle = new Paragraph(messageSource.getMessage("stats.player.title", null, locale), TITLE_FONT);
			mainTitle.setAlignment(Element.ALIGN_CENTER);
			mainTitle.setSpacingAfter(10);
			document.add(mainTitle);

			// Nombre del jugador
			Paragraph playerName = new Paragraph(messageSource.getMessage("stats.player", null, locale) + ": " + player.getUsername(), SUBTITLE_FONT);
			playerName.setAlignment(Element.ALIGN_CENTER);
			playerName.setSpacingAfter(20);
			document.add(playerName);

			// Resumen de estadísticas
			Paragraph summary = new Paragraph(messageSource.getMessage("stats.best.scores", null, locale), SUBTITLE_FONT);
			...
			addTableHeader(table, messageSource.getMessage("stats.trivia", null, locale));
			addTableHeader(table, messageSource.getMessage("stats.correct", null, locale));
			addTableHeader(table, messageSource.getMessage("stats.total", null, locale));
			addTableHeader(table, messageSource.getMessage("stats.time", null, locale));
			addTableHeader(table, messageSource.getMessage("stats.date", null, locale));
			...
			Paragraph statsTitle = new Paragraph(messageSource.getMessage("stats.global", null, locale), SUBTITLE_FONT);
			statsTitle.setSpacingBefore(10);
			statsTitle.setSpacingAfter(10);
			document.add(statsTitle);

			document.add(new Paragraph(messageSource.getMessage("stats.total.complete", null, locale) + " " + totalAttempts, NORMAL_FONT));
			final Object[] argsTotalCorrect = {totalCorrect, totalQuestions};
			document.add(new Paragraph(messageSource.getMessage("stats.total.correct.answer", argsTotalCorrect, locale), NORMAL_FONT));
			final String[] argsAvg = {String.format("%.2f", avgPercentage)};
			document.add(new Paragraph(messageSource.getMessage("stats.total.avg", argsAvg, locale), NORMAL_FONT));
	...
	private void addTriviaSection(Document document, TriviaWithQuestionsResponse trivia) throws DocumentException {
		...
		final String totalQuestionsTitle = messageSource.getMessage("trivia.report.total.questions", null, locale);
		String metaText = String.format("%s: %s | %s: %d", visibilityTitle, visibility, totalQuestionsTitle, trivia.questions().size());
